...
struct midas_machine_priv {
	...
	struct gpio_desc *gpio_submic_bias;
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	if (!priv->gpio_submic_bias)
		return 0;

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		gpiod_set_value_cansleep(priv->gpio_submic_bias, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
		gpiod_set_value_cansleep(priv->gpio_submic_bias, 0);
		break;
	}
...
static int midas_late_probe(struct snd_soc_card *card) {
	...
	if (ret < 0) {
		dev_err(codec->dev, "Failed to enable mclk1: %d\n", ret);
		return ret;
	}

	ret = clk_prepare_enable(priv->codec_mclk2);
	if (ret < 0) {
		dev_err(codec->dev, "Failed to enable mclk2: %d\n", ret);
		return ret;
	}
...
static int midas_probe(struct platform_device *pdev)
{
	...
	priv->gpio_submic_bias = devm_gpiod_get_optional(dev, "submic-bias",
							 GPIOD_OUT_HIGH);
	if (IS_ERR(priv->gpio_submic_bias)) {
		dev_err(dev, "Failed to get submic bias GPIO\n");
		return PTR_ERR(priv->gpio_submic_bias);
	}
